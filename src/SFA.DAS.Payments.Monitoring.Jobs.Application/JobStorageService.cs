    ...
    public class JobStorageService : IJobStorageService
    {
            ...
            IPaymentLogger logger)
        {
            this.stateManagerProvider =
                stateManagerProvider ?? throw new ArgumentNullException(nameof(stateManagerProvider));
            this.reliableTransactionProvider = reliableTransactionProvider ??
                                               throw new ArgumentNullException(nameof(reliableTransactionProvider));
        ...
        public async Task<bool> StoreNewJob(JobModel job, CancellationToken cancellationToken)
        {
            ...
            var cachedJob = await jobCache.TryGetValueAsync(reliableTransactionProvider.Current, job.DcJobId.Value,
            ...
            await jobCache.AddOrUpdateAsync(reliableTransactionProvider.Current, job.DcJobId.Value, id => job,
                (id, existingJob) => job, TransactionTimeout, cancellationToken);
        ...
        public async Task SaveJobStatus(long jobId, JobStatus jobStatus, DateTimeOffset endTime,
        ...
        {
            return await GetCurrentJobs(
                model => model.JobType == JobType.EarningsJob ||
                         model.JobType == JobType.ComponentAcceptanceTestEarningsJob, cancellationToken);
        }

        public async Task<List<long>> GetCurrentPeriodEndExcludingStartJobs(CancellationToken cancellationToken)
        {
            return await GetCurrentJobs(
                model => model.JobType == JobType.PeriodEndRunJob || model.JobType == JobType.PeriodEndStopJob ||
        ...
        public async Task StoreDcJobStatus(long jobId, bool succeeded, CancellationToken cancellationToken)
        {
            ...
            logger.LogDebug(
                $"Updating Job DcSubmissionStatus to {job.DcJobSucceeded} DcJobEndTime {job.DcJobEndTime}. Job: {job.DcJobId}");
        ...
        {
            var collection = await GetJobCollection();
            var item = await collection.TryGetValueAsync(reliableTransactionProvider.Current, jobId, TransactionTimeout,
                cancellationToken);
            return item.Value;
        }

        public async Task<List<InProgressMessage>> GetInProgressMessages(long jobId,
            CancellationToken cancellationToken)
        {
            ...
            var enumerator = (await inProgressCollection.CreateEnumerableAsync(reliableTransactionProvider.Current))
                .GetAsyncEnumerator();
            var identifiers = new List<InProgressMessage>();

            while (await enumerator.MoveNextAsync(cancellationToken)) identifiers.Add(enumerator.Current.Value);

            logger.LogInfo(
                $"Finished Getting {identifiers.Count} InProgressMessages, Elapsed Time {TimeSpan.FromTicks(stopwatch.ElapsedTicks).TotalSeconds} Seconds.");

            return identifiers;
        }

        public async Task RemoveInProgressMessages(long jobId, List<Guid> messageIdentifiers,
            CancellationToken cancellationToken)
        {
            ...
            logger.LogInfo(
                $"Finished Removing {messageIdentifiers.Count} InProgressMessages, Elapsed Time {TimeSpan.FromTicks(stopwatch.ElapsedTicks).TotalSeconds} Seconds.");
        }

        public async Task StoreInProgressMessages(long jobId, List<InProgressMessage> inProgressMessages,
            CancellationToken cancellationToken)
        {
            ...
            foreach (var inProgressMessage in inProgressMessages)
                await inProgressMessagesCollection.AddOrUpdateAsync(reliableTransactionProvider.Current,
                    inProgressMessage.MessageId,
                    key => inProgressMessage, (key, value) => inProgressMessage, TransactionTimeout,
                    cancellationToken);

            logger.LogInfo(
                $"Finished Storing {inProgressMessages.Count} InProgressMessages, Elapsed Time {TimeSpan.FromTicks(stopwatch.ElapsedTicks).TotalSeconds} Seconds.");
        }
        ...
        public async Task<List<CompletedMessage>> GetCompletedMessages(long jobId, CancellationToken cancellationToken)
        {
            ...
            var enumerator =
                (await completedMessageCollection.CreateEnumerableAsync(reliableTransactionProvider.Current))
                .GetAsyncEnumerator();
            ...
            logger.LogInfo(
                $"Finished Getting {identifiers.Count} CompletedMessages, Elapsed Time {TimeSpan.FromTicks(stopwatch.ElapsedTicks).TotalSeconds} Seconds.");

            return identifiers;
        }

        public async Task RemoveCompletedMessages(long jobId, List<Guid> completedMessages,
            CancellationToken cancellationToken)
        {
            ...
            logger.LogInfo(
                $"Finished Removing {completedMessages.Count} CompletedMessages, Elapsed Time {TimeSpan.FromTicks(stopwatch.ElapsedTicks).TotalSeconds} Seconds.");
        }
        ...
        public async Task StoreCompletedMessage(CompletedMessage completedMessage, CancellationToken cancellationToken)
        {
            ...
            logger.LogInfo(
                $"Finished Storing CompletedMessages {completedMessage.MessageId}, Elapsed Time {TimeSpan.FromTicks(stopwatch.ElapsedTicks).TotalSeconds} Seconds.");
        }

        public async Task<(bool hasFailedMessages, DateTimeOffset? endTime)> GetJobStatus(long jobId,
            CancellationToken cancellationToken)
        {
            var collection = await GetJobStatusCollection();
            var value = await collection.TryGetValueAsync(reliableTransactionProvider.Current, jobId,
                TransactionTimeout, cancellationToken);
            return value.HasValue ? value.Value : (hasFailedMessages: false, endTime: null);
        }

        public async Task StoreJobStatus(long jobId, bool hasFailedMessages, DateTimeOffset? endTime,
            CancellationToken cancellationToken)
        {
            var collection = await GetJobStatusCollection();
        ...
        public async Task SaveDataLocksCompletionTime(long jobId, DateTimeOffset endTime,
            CancellationToken cancellationToken)
        {
            await dataContext.SaveDataLocksCompletionTime(jobId, endTime, cancellationToken);
        }

        public async Task<List<long>> GetCurrentPeriodEndFcsHandoverJobs(CancellationToken cancellationToken)
        {
            return await GetCurrentJobs(model => model.JobType == JobType.PeriodEndFcsHandOverCompleteJob,
                cancellationToken);
        }
        ...
        private async Task<IReliableDictionary2<Guid, InProgressMessage>> GetInProgressMessagesCollection(long jobId)
        {
            return await stateManagerProvider.Current.GetOrAddAsync<IReliableDictionary2<Guid, InProgressMessage>>(
                $"{InProgressMessagesCacheKey}_{jobId}", TransactionTimeout);
        }
        ...
        private async Task<IReliableDictionary2<Guid, CompletedMessage>> GetCompletedMessagesCollection(long jobId)
        {
            return await stateManagerProvider.Current.GetOrAddAsync<IReliableDictionary2<Guid, CompletedMessage>>(
                $"{CompletedMessagesCacheKey}_{jobId}", TransactionTimeout);
        }

        private async Task<IReliableDictionary2<long, (bool hasFailedMessages, DateTimeOffset? endTime)>>
            GetJobStatusCollection()
        {
            return await stateManagerProvider.Current
                .GetOrAddAsync<IReliableDictionary2<long, (bool hasFailedMessages, DateTimeOffset? endTime)>>(
                    JobStatusCacheKey);
        }
    }
}