...
namespace SFA.DAS.Payments.Monitoring.Metrics.Application.PeriodEnd
{
    ...
    public class PeriodEndMetricsRepository : IPeriodEndMetricsRepository
    {
        ...
        public async Task<List<ProviderTransactionTypeAmounts>> GetTransactionTypesByContractType(short academicYear, byte collectionPeriod, CancellationToken cancellationToken)
        {
            ...
            {
                var transactionAmounts = await QueryDataContext.Payments
                    .Where(x => x.CollectionPeriod.AcademicYear == academicYear && x.CollectionPeriod.Period == collectionPeriod)
                    .Where(x => x.FundingPlatformType == null || x.FundingPlatformType == FundingPlatformType.SubmitLearnerData)
        ...
        public async Task<List<ProviderNegativeEarningsLearnerContractTypeAmounts>> GetPaymentAmountsForNegativeEarningsLearnersByContractType(List<long> learnerUlns, short academicYear, CancellationToken cancellationToken)
        {
            ...
            foreach (var batch in batches)
            {
                using (await QueryDataContext.BeginTransaction(cancellationToken))
                {
                    var amounts = await QueryDataContext.Payments
                        ...
                        .Where(p => p.FundingPlatformType == null || p.FundingPlatformType == FundingPlatformType.SubmitLearnerData)
                        .Select(x => new
                        {
                            x.Ukprn,
                            x.LearnerUln,
            ...
            return providerLearnerContractTypeAmounts;
        }
        ...
        public async Task<List<ProviderNegativeEarningsLearnerDataLockFundingLineTypeAmounts>> GetDataLockedAmountsForForNegativeEarningsLearners(List<long> learnerUlns, short academicYear, byte collectionPeriod,
