...
namespace SFA.DAS.Payments.Monitoring.Jobs.Client
{
    ...
    public class EarningsJobClient : IEarningsJobClient
    {
        ...
        public async Task StartJob(long jobId, long ukprn, DateTime ilrSubmissionTime, short collectionYear, byte collectionPeriod, List<GeneratedMessage> generatedMessages, DateTimeOffset startTime)
        {
            ...
            {
                logger.LogWarning(
                    $"Failed to send the request to record the earnings job. Job: {jobId}, Error: {ex.Message}. {ex}");
                throw;
            }
        }
        ...
        {
            logger.LogVerbose(
                $"Sending record job failed request to monitoring service for job: {jobId}, ukprn: {ukprn}");
            try
            {
                ...
                logger.LogDebug(
                    $"Sent record job status event for job: {jobId}, ukprn: {ukprn}. Status command: {typeof(T).Name}");
            }
            catch (Exception e)
            {
                logger.LogWarning(
                    $"Failed to send the request to record the failed job '{jobId}'. Error: {e.Message}. {e}");
                throw;
            }
        }
